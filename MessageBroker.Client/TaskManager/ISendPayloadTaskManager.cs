...
namespace MessageBroker.Client.TaskManager
{
    ...
    /// for such payloads we return a task that completes when either <see cref="Ok"/> or <see cref="Error"/> is received.
    /// for other types of payload that we don't need to wait for acknowledgment we can complete the task when the payload
    /// has been sent.
    /// </summary>
    public interface ISendPayloadTaskManager: IDisposable
    {
        /// <summary>
